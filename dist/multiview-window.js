const n = (f, v = {}, l = "") => {
  ...
  for (let w in v)
    i.setAttribute(w, v[w]);
  return i.innerHTML = l, i;
}, $ = (f, v) => (document.getElementById(f) || document.getElementsByTagName("head")[0].prepend(n("STYLE", { type: "text/css" }, v)), !0), B = function(f = {}) {
  const v = {
    map: "geocam-floating-window",
    split: "geocam-split-window",
    image: "geocam-base-window"
  };
  let l, i, w, s = null, e = f.mapElement, d = "map";
  ...
  this.setMapElement = function(a) {
    ...
    const x = function(u) {
      u.target.closest(".geocam-position-menu-wrapper") || h.classList.add("hidden");
    };
    ...
    }), h.addEventListener("click", function(u) {
      const y = u.target;
    ...
    H.addEventListener("click", l.hide), l.addControl(H, "left-top", { prepend: !0 }), d = o.get("mode") ? JSON.parse(o.get("mode")) : d, console.log("mode is", d), i = n("DIV", { class: `${v[d]}` }), w = n("DIV", { class: "geocam-window-move" });
    for (let u = 0; u < 9; u++)
      w.appendChild(
        n("DIV", {
          class: u == 4 ? "geocam-window-move-spacer" : "geocam-window-move-handler"
        })
      );
    V(), l.wrapper.parentNode.appendChild(i), i.appendChild(l.wrapper), l.visible(
      (u) => {
        u ? i.classList.remove("geocam-viewer-hidden") : i.classList.add("geocam-viewer-hidden");
      },
      { prepend: !0 }
...
class F extends HTMLElement {
  ...
  connectedCallback() {
    ...
    const v = this.closest("geocam-viewer");
    if (!v) {
      console.error(
    ...
    const l = () => {
      const i = v.viewer;
      if (i && typeof i.plugin == "function") {
        const w = this.getAttribute("target"), s = document.getElementById(w) || document.querySelector(w);
        if (!s) {
          console.error(`multiview-window: target '${w}' not found`);
          return;
        }
        this.viewer = i, this.plugin = new B({ mapElement: s }), this.viewer.plugin(this.plugin);
      } else
        setTimeout(l, 50);
    };
    l();
